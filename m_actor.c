...
void actorMachineGun(edict_t *self) {
    ...
    G_ProjectSource(self->s.origin,
            monster_flash_offset[MZ2_ACTOR_MACHINEGUN_1], forward, right,
            start);
    if (self->enemy) {
        if (self->enemy->health > 0) {
            VectorMA(self->enemy->s.origin, -0.2, self->enemy->velocity,
                    target);
...
void actor_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage,
        vec3_t point) {
    int n;
...
void target_actor_touch(edict_t *self, edict_t *other, cplane_t *plane,
    ...
    if (self->message) {
        ...
        for (n = 1; n <= game.maxclients; n++) {
            ...
            gi.cprintf(ent, PRINT_CHAT, "%s: %s\n",
                    actor_names[(other - g_edicts) % MAX_ACTOR_NAMES],
                    self->message);
        }
    }
    ...
    if (self->spawnflags & 1)		//jump
        ...
        if (other->groundentity) {
            ...
            gi.sound(other, CHAN_VOICE, gi.soundindex("player/male/jump1.wav"),
                    1, ATTN_NORM, 0);
        }
    }
