...
void get_settings(definition_item_t *defs, int num_defs) {
    ...
    S_GET_VAL(allow_voting_playersperteam, "allowvotingplayersperteam", defs,
            num_defs)
    ...
    S_GET_VAL(allow_voting_armorprotect, "allowvotingarmorprotect", defs,
            num_defs)
    S_GET_VAL(allow_voting_healthprotect, "allowvotinghealthprotect", defs,
            num_defs)
    S_GET_VAL(allow_voting_shotgun, "allowvotingshotgun", defs, num_defs)
    S_GET_VAL(allow_voting_supershotgun, "allowvotingsupershotgun", defs,
            num_defs)
    ...
    S_GET_VAL(allow_voting_grenadelauncher, "allowvotinggrenadelauncher", defs,
            num_defs)
    S_GET_VAL(allow_voting_rocketlauncher, "allowvotingrocketlauncher", defs,
            num_defs)
    S_GET_VAL(allow_voting_hyperblaster, "allowvotinghyperblaster", defs,
            num_defs)
    S_GET_VAL(allow_voting_railgun, "allowvotingrailgun", defs, num_defs)
    S_GET_VAL(allow_voting_bfg, "allowvotingbfg", defs, num_defs)
    S_GET_VAL(allow_voting_fallingdamage, "allowvotingfallingdamage", defs,
            num_defs)
...
void set_config(int stnum, int endnum) {
    ...
    for (i = stnum; i <= endnum; i++) {
        ...
        arenas[i].settings.allow_voting_playersperteam =
                allow_voting_playersperteam;
        arenas[i].settings.allow_voting_rounds = allow_voting_rounds;
        arenas[i].settings.allow_voting_maxteams = allow_voting_maxteams;
        arenas[i].settings.allow_voting_armorprotect =
                allow_voting_armorprotect;
        arenas[i].settings.allow_voting_healthprotect =
                allow_voting_healthprotect;
        arenas[i].settings.allow_voting_shotgun = allow_voting_shotgun;
        arenas[i].settings.allow_voting_supershotgun =
                allow_voting_supershotgun;
        ...
        arenas[i].settings.allow_voting_grenadelauncher =
                allow_voting_grenadelauncher;
        arenas[i].settings.allow_voting_rocketlauncher =
                allow_voting_rocketlauncher;
        arenas[i].settings.allow_voting_hyperblaster =
                allow_voting_hyperblaster;
        arenas[i].settings.allow_voting_railgun = allow_voting_railgun;
        arenas[i].settings.allow_voting_bfg = allow_voting_bfg;
        arenas[i].settings.allow_voting_fallingdamage =
                allow_voting_fallingdamage;
...
int ra_isalnum(char token) {
    ...
    i = (int) token;
    if ((i >= (int) '0' && i <= (int) '9') || (i >= (int) 'A' && i <= (int) 'Z')
            || (i >= (int) 'a' && i <= (int) 'z'))
        return 1;
    else
        return 0;
...
definition_item_t* new_def_block(void) {
    return ((definition_item_t*) gi.TagMalloc(
            sizeof(definition_item_t) * MAX_DEF_ITEMS, TAG_LEVEL));
}
...
//					curdef->keys = (char *)realloc(curdef->keys, strlen(curdef->keys) + 1);
                    ...
                    mode = MODE_VAL;
                } else if (*tok == '}') {	// end of block identifier
//					gi.dprintf("%d\n", level);

                    if (!level) {
                        gi.dprintf(
                                "Error reading config file: unbalanced {}\n");
                        return (0);
                    }
...
// NOTE: Change this if compiling for non-DOS!
    ...
    arena_blocks = (definition_item_t**) gi.TagMalloc(
            sizeof(definition_item_t*) * num_arenas, TAG_LEVEL);
    if (map = find_key(level.mapname, DEF_BLOCK, definition_blocks,
            num_definition_blocks)) {
        gi.dprintf("arena.cfg info for map found: %s\n", level.mapname);
        map_block = map;
        for (i = 0; i < num_arenas; i++) {
            arena = find_key(va("%d", i), DEF_BLOCK, map->val.defs,
                    map->numvals);
    ...
    if (it = find_key("votetries", DEF_VAL, definition_blocks,
            num_definition_blocks)) {
        votetries_setting = atoi(get_val(it->val.vals, 0));
    }

    if (it = find_key("grapple", DEF_VAL, definition_blocks,
            num_definition_blocks)) {
        allow_grapple = atoi(get_val(it->val.vals, 0));
    }

// get map loop
    if (map_loop = find_key("maploop", DEF_VAL, definition_blocks,
            num_definition_blocks)) {
