    ...
    if (logfile->value == 2) {
        ...
        if (attacker == self) {
            if (attacker->client->pers.weapon) {
                if (!strcmp(self->client->pers.weapon->classname,
                        "weapon_grenadelauncher")
                        || !strcmp(self->client->pers.weapon->classname,
                    ...
                    Com_sprintf(buf, sizeof(buf),
                            "%s\t\tSuicide\t\t-1\t%d\t%d\n",
                            self->client->pers.netname, (int) level.time,
                            self->client->ping);
                    GSdodeathlog(buf);
                }
            } else {
        ...
        if (attacker && attacker->client) {
            ...
            Com_sprintf(buf, sizeof(buf), "%s\t%s\tKill\t%s\t1\t%d\t%d\n",
                    ...
                    weaponname, (int) level.time, attacker->client->ping);
            GSdodeathlog(buf);
            GSCloseLog();
            return;
