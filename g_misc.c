...
void gib_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage,
        vec3_t point) {
    G_FreeEdict(self);
}
...
//arena
    if (self->arena > 0) {
        if (other->client->resp.team != -1)
            AddtoArena(other, self->arena, true, false);
        else
            menu_centerprint(other, "You must join or create a team first");
//			gi.centerprintf(other,"You must join or create a team first");
        return;
    }

    //allow observers to use teleporter
    //if (other->client->resp.flag != ARENA_PLAYER) return;
...
//ZOID
    ...
    //arena
    if (other->client->resp.flag == ARENA_PLAYER) {
...
void SP_misc_teleporter_dest(edict_t *ent) {
    ...
    ent->s.skinnum = 0;
    //        ent->solid = SOLID_BBOX;
//arena - get rid of model, not solid, not there!
    ent->svflags |= SVF_NOCLIENT;
    ent->solid = SOLID_NOT;
