...
int menuLeaveArena(edict_t *ent, arena_link_t *menulink, arena_link_t *selected,
    ...
    curstate =
            &arenas[((arena_team_t*) (teams[ent->client->resp.team].it))->context].game_state;
    if (*curstate != GAME_COUNT && *curstate != COMP_WAIT
            && ent->takedamage != DAMAGE_NO) //let observers leave
                    {
        menu_centerprint(ent,
                "Sorry, you cannot leave the arena\nduring a match");
        //		gi.centerprintf(ent, "Sorry, you cannot leave the arena\nduring a match");
        return 2;
    }
    remove_from_queue(
            &(((arena_team_t*) (teams[ent->client->resp.team].it))->ar), NULL); //take them out of whatever line their in
 ...
 *******/
int menuLeaveTeamAr(edict_t *ent, arena_link_t *menulink,
 ...
 *******/
int menuStepInOutofLine(edict_t *ent, arena_link_t *menulink,
        arena_link_t *selected, int key) {
    int arena;
    ...
    observeonly =
            !((arena_team_t*) (teams[ent->client->resp.team].it))->observeonly;
 ...
 *******/
int menuShowSettingsPropose(edict_t *ent, arena_link_t *menulink,
        arena_link_t *selected, int key) {
    int timeleft;

    if (arenas[ent->client->resp.context].voteinfo.endtime > level.time) {
        timeleft = (int) (arenas[ent->client->resp.context].voteinfo.endtime
                - level.time);
        if (timeleft < 30)
            menu_centerprint(ent,
                    va("Voting is in progress.\nPlease wait %d seconds",
                            (int) (arenas[ent->client->resp.context].voteinfo.endtime
                                    - level.time)));
    ...
    if (!ent->client->resp.votetries) {
        menu_centerprint(ent,
                va(
                        "Sorry, you cannot propose any more changes.\nYou have already proposed %d times\n",
                        votetries_setting));
 ...
 *******/
int menuShowSettingsVote(edict_t *ent, arena_link_t *menulink,
        arena_link_t *selected, int key) {
    if (arenas[ent->client->resp.context].voteinfo.endtime < level.time) {
...
void show_observer_menu(edict_t *ent) {
    ...
    if (!((arena_team_t*) (teams[ent->client->resp.team].it))->observeonly) {
        AddMenuItem(menul, "Change Arena Settings", NULL, -1,
                &menuShowSettingsPropose);
...
void show_arena_menu(edict_t *ent) {
    ...
    for (i = 1; i <= num_arenas; i++) {
        if (arenas[i].pickup)
            ...
            AddMenuItem(menul, getarenaname(i), " T:",
                    count_queue(&arenas[i].unassigned)
                            + count_queue(&arenas[i].players), &menuAddtoArena);
    }
    AddMenuItem(menul, "", NULL, -1, NULL);
...
int menuAddtoTeam(edict_t *ent, arena_link_t *menulink, arena_link_t *selected,
    ...
    if (add_to_team(ent, ((menu_item_t*) selected->it)->itemtext)) {
        if (!((arena_team_t*) (teams[ent->client->resp.team].it))->context)
            ...
            move_to_arena(ent,
                    ((arena_team_t*) (teams[ent->client->resp.team].it))->context,
                    true);
        }
        return 0;
    } else {
        menu_centerprint(ent,
                "That team is already in an arena\nand full or\nthe arena is locked");
...
int menuRefreshTeamList(edict_t *ent, arena_link_t *menulink,
    ...
    for (i = 0; i < MAXTEAMS; i++) {
        if (teams[i].it)
            AddMenuItem(menul, ((arena_team_t*) (teams[i].it))->teamname,
                    " Players: ", count_queue(&teams[i]), &menuAddtoTeam);
    }
 ...
 ********/
int menuChangeProtect(edict_t *ent, arena_link_t *menulink,
        arena_link_t *selected, int key) {
    int val;
    char *str;
...
int menuChangeMap(edict_t *ent, arena_link_t *menulink, arena_link_t *selected,
        int key) {
    strcpy(((menu_item_t*) selected->it)->valuetext,
            get_next_map(((menu_item_t*) selected->it)->valuetext));
    return 1;
}
...
void Cmd_admin_f(edict_t *ent) {
    ...
    if (code == admincode->value) {
        ...
        AddMenuItem(menul, "Fraglimit:        ", NULL, fraglimit->value,
                &menuChangeValue);
        AddMenuItem(menul, "Timelimit:        ", NULL, timelimit->value,
                &menuChangeValue);
        iteml = AddMenuItem(menul, "Mapname:          ",
                "                                ", -1, &menuChangeMap);
    ...
    while (itemlink->next) {
        ...
        if (Q_stricmp(menuitem->itemtext, "Arena:                 ") == 0) {
            ...
            if (((menu_item_t*) selected->it)->itemtext[0] == 'A') //force apply
                ...
                memcpy(&arenas[arena].voteinfo.propsettings,
                        &arenas[arena].settings, sizeof(arena_settings_t));
            ...
            if (!mysettings->allow_voting_shotgun)
                newweapons |=
                        (mysettings->weapons & weapon_vals[0]) ?
                                weapon_vals[0] : 0;
            if (!mysettings->allow_voting_supershotgun)
                newweapons |=
                        (mysettings->weapons & weapon_vals[1]) ?
                                weapon_vals[1] : 0;
            if (!mysettings->allow_voting_machinegun)
                newweapons |=
                        (mysettings->weapons & weapon_vals[2]) ?
                                weapon_vals[2] : 0;
            if (!mysettings->allow_voting_chaingun)
                newweapons |=
                        (mysettings->weapons & weapon_vals[3]) ?
                                weapon_vals[3] : 0;
            if (!mysettings->allow_voting_grenadelauncher)
                newweapons |=
                        (mysettings->weapons & weapon_vals[4]) ?
                                weapon_vals[4] : 0;
            if (!mysettings->allow_voting_rocketlauncher)
                newweapons |=
                        (mysettings->weapons & weapon_vals[5]) ?
                                weapon_vals[5] : 0;
            if (!mysettings->allow_voting_hyperblaster)
                newweapons |=
                        (mysettings->weapons & weapon_vals[6]) ?
                                weapon_vals[6] : 0;
            if (!mysettings->allow_voting_railgun)
                newweapons |=
                        (mysettings->weapons & weapon_vals[7]) ?
                                weapon_vals[7] : 0;
            if (!mysettings->allow_voting_bfg)
                newweapons |=
                        (mysettings->weapons & weapon_vals[8]) ?
                                weapon_vals[8] : 0;
            mysettings->weapons = newweapons;
        } else if (Q_stricmp(menuitem->itemtext, "Players per team:      ")
                == 0) {
            mysettings->needed_players_team = menuitem->itemvalue;
        } else if (Q_stricmp(menuitem->itemtext, "Initial Health:        ")
                == 0) {
            mysettings->health = menuitem->itemvalue;
        } else if (Q_stricmp(menuitem->itemtext, "Initial Armor:         ")
                == 0) {
            mysettings->armor = menuitem->itemvalue;
        } else if (Q_stricmp(menuitem->itemtext, "Minimum Ping:          ")
                == 0) {
            mysettings->minping = menuitem->itemvalue;
        } else if (Q_stricmp(menuitem->itemtext, "Maximum Ping:          ")
                == 0) {
            mysettings->maxping = menuitem->itemvalue;
        } else if (Q_stricmp(menuitem->itemtext, "Rounds:                ")
                == 0) {
            mysettings->rounds = (menuitem->itemvalue / 2) * 2 + 1; //always odd!
        } else if (Q_stricmp(menuitem->itemtext, "Allow Shotgun:         ")
                == 0) {
            mysettings->weapons |=
                    (menuitem->valuetext[0] == 'Y') ? weapon_vals[0] : 0;
        } else if (Q_stricmp(menuitem->itemtext, "Allow Super Shotgun:   ")
                == 0) {
            mysettings->weapons |=
                    (menuitem->valuetext[0] == 'Y') ? weapon_vals[1] : 0;
        } else if (Q_stricmp(menuitem->itemtext, "Allow Machine gun:     ")
                == 0) {
            mysettings->weapons |=
                    (menuitem->valuetext[0] == 'Y') ? weapon_vals[2] : 0;
        } else if (Q_stricmp(menuitem->itemtext, "Allow Chain gun:       ")
                == 0) {
            mysettings->weapons |=
                    (menuitem->valuetext[0] == 'Y') ? weapon_vals[3] : 0;
        } else if (Q_stricmp(menuitem->itemtext, "Allow Grenade Launcher:")
                == 0) {
            mysettings->weapons |=
                    (menuitem->valuetext[0] == 'Y') ? weapon_vals[4] : 0;
        } else if (Q_stricmp(menuitem->itemtext, "Allow Rocket Launcher: ")
                == 0) {
            mysettings->weapons |=
                    (menuitem->valuetext[0] == 'Y') ? weapon_vals[5] : 0;
        } else if (Q_stricmp(menuitem->itemtext, "Allow Hyperblaster:    ")
                == 0) {
            mysettings->weapons |=
                    (menuitem->valuetext[0] == 'Y') ? weapon_vals[6] : 0;
        } else if (Q_stricmp(menuitem->itemtext, "Allow Railgun:         ")
                == 0) {
            mysettings->weapons |=
                    (menuitem->valuetext[0] == 'Y') ? weapon_vals[7] : 0;
        } else if (Q_stricmp(menuitem->itemtext, "Allow BFG10K:          ")
                == 0) {
            mysettings->weapons |=
                    (menuitem->valuetext[0] == 'Y') ? weapon_vals[8] : 0;
        ...
        } else if (Q_stricmp(menuitem->itemtext, "Falling Damage:        ")
                == 0) {
            mysettings->fallingdamage = (menuitem->valuetext[0] == 'Y');
        } else if (Q_stricmp(menuitem->itemtext, "Lock Arena:            ")
                == 0) {
...
/*******
 menuVote Yes/No
 ********/
int menuVote(edict_t *ent, arena_link_t *menulink, arena_link_t *selected,
        int key) {
    int arena;
...
void Cmd_arenaadmin_f(edict_t *ent, adminmenu_mode_t mode) {
    ...
    arena_link_t *menul;
    int arena = 0;
    int (*mymenuChangeValue)(edict_t *ent, arena_link_t *menulink,
            arena_link_t *selected, int key);
    int (*mymenuChangeValue10)(edict_t *ent, arena_link_t *menulink,
            arena_link_t *selected, int key);
    int (*mymenuChangeYesNo)(edict_t *ent, arena_link_t *menulink,
            arena_link_t *selected, int key);
    int (*mymenuChangeProtect)(edict_t *ent, arena_link_t *menulink,
            arena_link_t *selected, int key);
    ...
    case VOTE:
        ...
        if (arena < 1 || arena > num_arenas)
            return;
        mymenuChangeValue = mymenuChangeValue10 = mymenuChangeYesNo =
                mymenuChangeProtect = NULL;
        ...
        if (arenas[arena].pickup != 1) {
            if (mysettings->needed_players_team
                    != cursettings->needed_players_team)
                AddMenuItem(menul, "Players per team:      ", NULL,
                        mysettings->needed_players_team, NULL);
        }
        if (mysettings->health != cursettings->health)
            AddMenuItem(menul, "Initial Health:        ", NULL,
                    mysettings->health, mymenuChangeValue10);
        if (mysettings->armor != cursettings->armor)
            AddMenuItem(menul, "Initial Armor:         ", NULL,
                    mysettings->armor, mymenuChangeValue10);
        if (arenas[arena].pickup != 1) {
            if (mysettings->minping != cursettings->minping)
                AddMenuItem(menul, "Minimum Ping:          ", NULL,
                        mysettings->minping, mymenuChangeValue10);
            if (mysettings->maxping != cursettings->maxping)
                AddMenuItem(menul, "Maximum Ping:          ", NULL,
                        mysettings->maxping, mymenuChangeValue10);
        }
        if (mysettings->rounds != cursettings->rounds)
            AddMenuItem(menul, "Rounds:                ", NULL,
                    mysettings->rounds, mymenuChangeValue);
        if ((mysettings->weapons & weapon_vals[0])
                != (cursettings->weapons & weapon_vals[0]))
            AddMenuItem(menul, "Allow Shotgun:         ",
                    (mysettings->weapons & weapon_vals[0]) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        if ((mysettings->weapons & weapon_vals[1])
                != (cursettings->weapons & weapon_vals[1]))
            AddMenuItem(menul, "Allow Super Shotgun:   ",
                    (mysettings->weapons & weapon_vals[1]) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        if ((mysettings->weapons & weapon_vals[2])
                != (cursettings->weapons & weapon_vals[2]))
            AddMenuItem(menul, "Allow Machine gun:     ",
                    (mysettings->weapons & weapon_vals[2]) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        if ((mysettings->weapons & weapon_vals[3])
                != (cursettings->weapons & weapon_vals[3]))
            AddMenuItem(menul, "Allow Chain gun:       ",
                    (mysettings->weapons & weapon_vals[3]) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        if ((mysettings->weapons & weapon_vals[4])
                != (cursettings->weapons & weapon_vals[4]))
            AddMenuItem(menul, "Allow Grenade Launcher:",
                    (mysettings->weapons & weapon_vals[4]) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        if ((mysettings->weapons & weapon_vals[5])
                != (cursettings->weapons & weapon_vals[5]))
            AddMenuItem(menul, "Allow Rocket Launcher: ",
                    (mysettings->weapons & weapon_vals[5]) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        if ((mysettings->weapons & weapon_vals[6])
                != (cursettings->weapons & weapon_vals[6]))
            AddMenuItem(menul, "Allow Hyperblaster:    ",
                    (mysettings->weapons & weapon_vals[6]) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        if ((mysettings->weapons & weapon_vals[7])
                != (cursettings->weapons & weapon_vals[7]))
            AddMenuItem(menul, "Allow Railgun:         ",
                    (mysettings->weapons & weapon_vals[7]) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        if ((mysettings->weapons & weapon_vals[8])
                != (cursettings->weapons & weapon_vals[8]))
            AddMenuItem(menul, "Allow BFG10K:          ",
                    (mysettings->weapons & weapon_vals[8]) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        if (mysettings->healthprotect != cursettings->healthprotect)
            AddMenuItem(menul, "Health: ",
                    StringForProtect(mysettings->healthprotect), -1,
                    mymenuChangeProtect);
        if (mysettings->armorprotect != cursettings->armorprotect)
            AddMenuItem(menul, "Armor:  ",
                    StringForProtect(mysettings->armorprotect), -1,
                    mymenuChangeProtect);
        if (mysettings->fallingdamage != cursettings->fallingdamage)
            AddMenuItem(menul, "Falling Damage:        ",
                    (mysettings->fallingdamage) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        AddMenuItem(menul, "", NULL, -1, NULL);
        break;
    }
    if (mode != VOTE) {
        ...
        if (arenas[arena].pickup != 1) {
            if (mode == FORCE || mysettings->allow_voting_playersperteam)
                AddMenuItem(menul, "Players per team:      ", NULL,
                        mysettings->needed_players_team, mymenuChangeValue);
        }
        ...
        if (mode == FORCE || mysettings->allow_voting_rounds)
            AddMenuItem(menul, "Rounds:                ", NULL,
                    mysettings->rounds, mymenuChangeValue); //have to make NO at least 3 chars so that when you strcpy YES into it it fits
        ...
        if (mode == FORCE || mysettings->allow_voting_fallingdamage)
            AddMenuItem(menul, "Falling Damage:        ",
                    ...
                    mymenuChangeYesNo);
        if (mode == FORCE)
            AddMenuItem(menul, "Lock Arena:            ",
                    (mysettings->lock_arena) ? "YES" : "NO ", -1,
                    mymenuChangeYesNo);
        AddMenuItem(menul, "", NULL, -1, NULL);
    }
    switch (mode) {
 ...
 ********/
int menuMotdContinue(edict_t *ent, arena_link_t *menulink,
        arena_link_t *selected, int key) {
...
void motd_menu(edict_t *ent) {
    ...
    //AddMenuItem(menul, "", NULL, -1, NULL);
    AddMenuItem(menul, "---------Continue----------", NULL, -1,
            &menuMotdContinue);
...
int menuNo(edict_t *ent, arena_link_t *menulink, arena_link_t *selected,
        int key) {
    return 0;
}
...
void show_teamconfirm_menu(edict_t *ent, int arena) {
    ...
    AddMenuItem(menul, "", NULL, -1, NULL);
    AddMenuItem(menul, "Yes, continue to arena ", NULL, arena,
            &menuTeamConfirm);
