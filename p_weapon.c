...
//arena
	...
	if (ent->client->weaponstate == WEAPON_DROPPING)
	{
		...
		//arena ### Hentai ### BEGIN
		else if(ent->client->vwepmodel && (FRAME_DEACTIVATE_LAST - ent->client->ps.gunframe) == 4)
		{
	...
	{
		//arena
		if (ent->client->ps.gunframe == FRAME_ACTIVATE_LAST || arenas[ent->client->resp.context].settings.fastswitch)
		{
	...
	if ((ent->client->newweapon) && (ent->client->weaponstate != WEAPON_FIRING))
	{
		...
		//arena
		if (arenas[ent->client->resp.context].settings.fastswitch)
			ent->client->ps.gunframe = FRAME_DEACTIVATE_LAST;
		else
			ent->client->ps.gunframe = FRAME_DEACTIVATE_FIRST;
		// arena ### Hentai ### BEGIN
		if(ent->client->vwepmodel && (FRAME_DEACTIVATE_LAST - FRAME_DEACTIVATE_FIRST) < 4)
		{
			ent->client->anim_priority = ANIM_REVERSE;
			if(ent->client->ps.pmove.pm_flags & PMF_DUCKED)
			{
...
void Weapon_RocketLauncher_Fire (edict_t *ent)
{
	...
	VectorSet(offset, 8, 8, ent->viewheight-8);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
	//arena   fire_rocket (ent, start, forward, damage, 550, damage_radius, radius_damage);
	fire_rocket (ent, start, forward, damage, arenas[ent->client->resp.context].settings.rocket_speed, damage_radius, radius_damage);
