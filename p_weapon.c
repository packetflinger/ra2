...
void ChangeWeapon(edict_t *ent) {
    ...
    if (ent->client->pers.weapon && ent->client->pers.weapon->ammo)
        ent->client->ammo_index = ITEM_INDEX(
                FindItem(ent->client->pers.weapon->ammo));
    else
        ent->client->ammo_index = 0;
//arena
    if (!ent->client->pers.weapon || ent->s.modelindex != 255) {	// dead
...
//arena
    ...
    if (ent->client->weaponstate == WEAPON_ACTIVATING) {
        ...
        if (ent->client->ps.gunframe == FRAME_ACTIVATE_LAST
                || arenas[ent->client->resp.context].settings.fastswitch) {
    ...
    if (ent->client->weaponstate == WEAPON_READY) {
        if (((ent->client->latched_buttons | ent->client->buttons)
                ...
                &&
                //arena
                ent->takedamage != DAMAGE_NO && ent->client
...
void Weapon_RocketLauncher_Fire(edict_t *ent) {
    ...
    fire_rocket(ent, start, forward, damage,
            arenas[ent->client->resp.context].settings.rocket_speed,
...
void Weapon_HyperBlaster_Fire(edict_t *ent) {
    ...
    //arena
    if (!(ent->client->buttons & BUTTON_ATTACK) || ent->takedamage == DAMAGE_NO
            || ent->client
                    && arenas[ent->client->resp.context].game_state
                            != GAME_ON) {
        ent->client->ps.gunframe++;
    } else {
        if (!ent->client->pers.inventory[ent->client->ammo_index]) {
