...
static int CheckArmor(edict_t *ent, vec3_t point, vec3_t normal, int damage,
        int te_sparks, int dflags, edict_t *attacker) {
    gclient_t *client;
    int save, save2;
    int index;
    gitem_t *armor;
...
// arena
    // teamplay armor damage avoidance
    save2 = save;
    if (OnSameTeam(ent, attacker) && !(dflags & DAMAGE_NO_PROTECTION)) {
        if (arenas[client->resp.context].settings.armorprotect == 1) {
            save2 = 0;
        } else if (arenas[client->resp.context].settings.armorprotect == 2
                && ent != attacker) {
            save2 = 0;
        }
    }
    client->pers.inventory[index] -= save2;
    SpawnDamage(te_sparks, point, normal, save2);
...
// arena
    ...
    client = targ->client;
    /* arena ### Hentai ### BEGIN  -- damage multiplier crap
     if (client && client->vwepmodel)
