...
//        deathmatch = gi.cvar ("deathmatch", "0", CVAR_SERVERINFO|CVAR_LATCH);
	...
	logfile = gi.cvar("logfile","0",CVAR_SERVERINFO); //GSLogMod arena
	ispublic = gi.cvar("public","1",0); //arena
	netlog = gi.cvar("netlog","",CVAR_SERVERINFO);//arena
	if (ispublic->value == 0)
		gi.cvar_set("netlog","");
	...
	for (i=0 ; i<game.maxclients ; i++)
	{
		...
		//arena - work around for reconnect w/o disconnect
