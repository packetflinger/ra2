...
void ClientObituary (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
	...
	if (attacker && attacker->client)
	{
		//arena
				...
				if (attacker->health >= arenas[attacker->client->resp.context].settings.health)
				{
					if(self->health < -40) 
				...
				else if (attacker->health >= arenas[attacker->client->resp.context].settings.health - 20)
				{
...
//arena - fix lastframe bug
	self->client->grenade_time = 0;
        self->client->resp.telefrag_time = 0;
...
//arena
	client->resp.votetries = votetries_setting;
	client->resp.team = -1;
	client->resp.context = 0;
	client->resp.flag = ARENA_OBSERVER;
	client->resp.omode = NORMAL;
	client->resp.ar.next = NULL;
	client->resp.ar.prev = NULL;
	client->resp.initialized = 1;
	client->resp.isbot = 0;
	if (client->cport == 27902)
		client->resp.isbot = BOT_BYPORT;
		
	client->resp.zbotcount = 0;
	client->resp.zbottime = 0;
}
...
void InitBodyQue (void)
{
	...
	for (i=0; i<BODY_QUEUE_SIZE ; i++)
	{
		...
		//arena -- crasing with movetype 4 on bodies?
...
// arena ent->takedamage = DAMAGE_AIM;
        ent->takedamage = DAMAGE_NO;            // set to yes when the match starts
...
*/
	...
	ChangeWeapon (ent);
	//arena
	ent->client->resp.telefrag_time = 0;        
	...
	ent->client->resp.context = 0; //need to clear so observer menu will come up
...
//arena.. dont let them spawn in during intermission
...
//arena zbot
	...
	if (*s) //found a :
	{
		...
		{
			gi.dprintf("\n%s\nConnected with ZBOT\n",userinfo);
			ent->client->resp.isbot = true;
		}
	}
*/
//arena
	if (!strstr(s,"/nullxxx")) //if we didn't set it..
	{
		if (ent->client->resp.team != -1 && (((arena_team_t *)(teams[ent->client->resp.team].it))->skin != -1) )
		{
			//strcpy (ent->client->pers.sounddir, "player/male");
			setteamskin(ent,userinfo,((arena_team_t *)(teams[ent->client->resp.team].it))->skin);
			/*		playernum = ent-g_edicts-1;
			gi.configstring (CS_PLAYERSKINS+playernum, va("%s\\male/%s", ent->client->pers.netname, teamskins[ent->client->resp.team % NUMSKINS]) );
			*/
		} else
	...
	{
		s = Info_ValueForKey (ent->client->pers.userinfo, "skin"); //we need to let them make other changes, but we cant let the nullxxx skin get set in userinfo
		Info_RemoveKey(userinfo,"skin");
		if (ent->client->resp.team == -1 || (((arena_team_t *)(teams[ent->client->resp.team].it))->skin == -1) )
		{ //not on a pickup team any more
			s = "male/grunt";
			gi.configstring (CS_PLAYERSKINS+playernum, va("%s\\%s", ent->client->pers.netname, s) );
		}
	...
	//arena
	s = Info_ValueForKey (userinfo, "skin");
	ent->client->vwepmodel = checkvwepmodel(s);
	// fov
	if (deathmatch->value && ((int)dmflags->value & DF_FIXED_FOV))
	{
...
qboolean ClientConnect (edict_t *ent, char *userinfo)
{
	...
	//arena reconnected w/o disconnecting
...
//			return false;
	...
	if (ent->inuse == false)
	{
		...
		InitClientResp (ent->client);
	//arena ??	if (!game.autosaved || !ent->client->pers.weapon)
...
//arena
	...
	{	
		ent->client->resp.isbot = BOT_BYIMP;
	}
	if (!ent->client->resp.isbot)
	{
		if (level.time - ent->client->resp.zbottime > 5) //more than 5 seconds
				ent->client->resp.zbotcount = 0;
		if (ent->client->oldangles[0][0] == ucmd->angles[0] &&
			ent->client->oldangles[1][0] != ucmd->angles[0] &&
			ent->client->oldangles[0][1] == ucmd->angles[1] &&
			ent->client->oldangles[1][1] != ucmd->angles[1])
			//ent->client->resp.flag == arena_player )// && they are
		{
			ent->client->resp.zbottime = level.time;
		...
		ent->client->oldangles[1][1] = ucmd->angles[1];
		if (ent->client->resp.zbotcount > 10) //gotta be a zbot!
			ent->client->resp.isbot = BOT_BYANGLES;
	}
	if(ent->client->resp.track_targ &&
		ent->client->resp.flag == ARENA_OBSERVER) { // we're tracking
		client->ps.pmove.pm_type = PM_GIB;	// a guess: will this allow movement but cant' be controlled by player?
		client->ps.pmove.gravity = 0;		// floating

		if(ent->client->resp.omode == TRACKCAM) {
			track_think(ent, ucmd);
		} else if (ent->client->resp.omode == EYECAM) {
...
//arena
	//enter trackcam from freeflying mode when attack pressed, or exit it
	if(!(client->latched_buttons & BUTTON_ATTACK)) {
		client->resp.track_flag = client->resp.track_flag & ~(1);
	} else
	if(client->resp.flag == ARENA_OBSERVER &&
	   client->resp.context != 0 &&
	   client->latched_buttons & BUTTON_ATTACK) {
		if(!(client->resp.track_flag & 1)) {
			//menuChangeOMode(ent, NULL,NULL, 0);
			ChangeOMode(ent);
			client->resp.track_flag |= 1;
		}
	}
	if(!(ucmd->upmove)) {
		client->resp.track_flag = client->resp.track_flag & ~(2);
	}
	if(client->resp.flag == ARENA_OBSERVER &&
	   (client->resp.omode == TRACKCAM || client->resp.omode == EYECAM) &&
	   ucmd->upmove) { // track next person
		if(!(client->resp.track_flag & 2)) {
			if (ucmd->upmove > 0)
				track_next(ent);
			else
				track_prev(ent);
			client->resp.track_flag |= 2;
		}
	}
