...
typedef enum {
    DAMAGE_NO, DAMAGE_YES,  // will take damage if hit
...
extern cvar_t *logfile;		//GSLog arena
extern cvar_t *netlog; //arena
...
//arena
    int team;
    int menu;
    arena_player_t flag;    // whether they are assigned or not
    int context;            // the arena # this person belongs to
    arena_link_t ar; //to link them in line
    int telefrag_time;
    observer_mode_t omode;
    int initialized;
    edict_t *track_targ;
    int track_flag;
    observer_mode_t old_omode;
    qboolean voted;
    int votetries;			// vote spam protection
    int zbotcount;
    float zbottime;
    int isbot;
    /*
...
// arena
    int cport; //connected port, for zbot check
    short oldangles[2][2]; //for zbot check
