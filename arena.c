...
char *teamskins[] = { "r2red", "r2blue", "r2dgre", "r2oran", "r2yell", "r2aqua",
        "r2lgre" };
char *vwepmodels[] = { "male", "female", "cyborg", "crakhor" };
int teamskins_precachem[NUMSKINS], teamskins_precachef[NUMSKINS],
        teamskins_precachecw[NUMSKINS], teamskins_precachecb[NUMSKINS];
/* todo:
        ...
        if (a->settings.weapons & weapon_vals_x[i]) {
            ...
            if (!e->client->pers.inventory[ITEM_INDEX(rl)] || needswitch) {
                ...
                e->client->pers.selected_item =
                        e->client->ps.stats[STAT_SELECTED_ITEM] = ITEM_INDEX(
                                rl);
                needswitch = false;
            }
...
// give ammo
    if (it = FindItemByClassname("ammo_shells"))
        e->client->pers.inventory[ITEM_INDEX(it)] = a->settings.shells;	//shells	100
    if (it = FindItemByClassname("ammo_bullets"))
        e->client->pers.inventory[ITEM_INDEX(it)] = a->settings.bullets;//bullets       200
    if (it = FindItemByClassname("ammo_slugs"))
        e->client->pers.inventory[ITEM_INDEX(it)] = a->settings.slugs;//slugs	 50
    if (it = FindItemByClassname("ammo_grenades"))
        e->client->pers.inventory[ITEM_INDEX(it)] = a->settings.grenades;//grenades      50
    if (it = FindItemByClassname("ammo_rockets"))
        e->client->pers.inventory[ITEM_INDEX(it)] = a->settings.rockets;//rockets       50
    if (it = FindItemByClassname("ammo_cells"))
        e->client->pers.inventory[ITEM_INDEX(it)] = a->settings.cells;//cells	 200
// give body armor
    ...
    for (i = 0; i < MAXTEAMS; i++) {
        if (teams[i].it) {
            ...
            if (!strcmp(ateam->teamname, teamname)) {
                ...
                if (ateam->context) {
                    if (count_queue(&teams[i])
                            == arenas[ateam->context].settings.needed_players_team)
...
void track_SetStats(edict_t *ent) {
    ...
    memcpy(ent->client->ps.stats, targ->client->ps.stats,
            sizeof(targ->client->ps.stats[0]) * MAX_STATS);
...
void eyecam_think(edict_t *ent, usercmd_t *ucmd) {
    ...
    for (i = 0; i < 3; i++)
        ent->client->ps.pmove.delta_angles[i] = ANGLE2SHORT(
                ent->s.angles[i] - ent->client->resp.cmd_angles[i]);
...
void track_think(edict_t *ent, usercmd_t *ucmd) {
    ...
    trace = gi.trace(ent->s.origin, mins, maxs, ent->s.origin, ent,
            MASK_PLAYERSOLID);
    if (trace.contents & (CONTENTS_SOLID | CONTENTS_WINDOW))
        blocked = true;
    if (!blocked)
...
void track_change(edict_t *ent, int value) {
    ...
    if (!targ) {
        targ = g_edicts + 1;
        fail = true;
    } else if (targ->client->resp.flag != ARENA_PLAYER
    ...
    do {
        ...
        if (pos->client->resp.flag != ARENA_PLAYER
                || pos->client->resp.context != ent->client->resp.context)
            continue;
...
void SetObserverMode(edict_t *ent) {
    ...
    case TRACKCAM:
        ...
        for (i = 0; i < 3; i++) {
            ent->client->ps.pmove.delta_angles[i] = ANGLE2SHORT(
                    0 - ent->client->resp.cmd_angles[i]);
            ent->s.angles[i] = 0;
        }
        VectorCopy(ent->s.angles, ent->client->ps.viewangles);
        VectorCopy(ent->s.angles, ent->client->v_angle);
        if (!ent->client->resp.track_targ
                || ent->client->resp.track_targ->client->resp.flag
                        != ARENA_PLAYER)
            track_next(ent);
        break;
    case EYECAM:
...
//arena zbot
    if (ent->client->resp.isbot) {
        gi.dprintf("\n%s IS A ZBOT %d\n", ent->client->pers.netname,
                ent->client->resp.isbot);
        //tell them
        gi.centerprintf(ent,
                "The server seems to think you\nare a bot. If you aren't,\n you may wish to reconnect");
    }
    ...
    if (observer) {
        if (!arenas[arena].idarena)
            ...
            dest = SelectFarthestArenaSpawnPoint("info_player_deathmatch",
                    arena);
        ...
        if (arenas[arena].pickup)
            dest = SelectRandomArenaSpawnPoint("info_player_deathmatch", arena,
                    (((arena_team_t*) teams[ent->client->resp.team].it)->side
                            == arenas[arena].eventeam) ? 1 : 2);
        else
            dest = SelectFarthestArenaSpawnPoint("info_player_deathmatch",
                    arena);
    }
    ...
    for (i = 0; i < 3; i++)
        ent->client->ps.pmove.delta_angles[i] = ANGLE2SHORT(
                dest->s.angles[i] - ent->client->resp.cmd_angles[i]);
    ...
    if (arenas[arena].voteinfo.endtime > level.time && !ent->client->resp.voted) //inform them of vote in progress
        ...
        menu_centerprint(ent,
                "Settings changes have been proposed!\nGoto the observer menu (TAB) to vote");
    ...
    if (s[0] == 'f') {
        if (strcmp(s, va("female/%s", teamskins[skinnum])))
            gi.configstring(CS_PLAYERSKINS + playernum,
                    va("%s\\female/%s", ent->client->pers.netname,
                            teamskins[skinnum]));
        ...
        if (strcmp(s, va("crakhor/%s", teamskins[skinnum])))
            gi.configstring(CS_PLAYERSKINS + playernum,
                    va("%s\\crakhor/%s", ent->client->pers.netname,
                            teamskins[skinnum]));
        Info_RemoveKey(userinfo, "skin");
        ...
        if (strcmp(s, va("cyborg/%s", teamskins[skinnum])))
            gi.configstring(CS_PLAYERSKINS + playernum,
                    va("%s\\cyborg/%s", ent->client->pers.netname,
                            teamskins[skinnum]));
        Info_RemoveKey(userinfo, "skin");
        ...
        if (strcmp(s, va("male/%s", teamskins[skinnum]))) //if it changed
            gi.configstring(CS_PLAYERSKINS + playernum,
                    va("%s\\male/%s", ent->client->pers.netname,
                            teamskins[skinnum]));
        Info_RemoveKey(userinfo, "skin");
...
void SendTeamToArena(arena_link_t *team, int arena, qboolean observe,
    ...
    if (!((arena_team_t*) team->it)->observeonly) {
        if (arena != 0 && ((arena_team_t*) team->it)->skin == -1
                && (arenas[arena].settings.needed_players_team > 1
                        || arenas[arena].pickup))
    ...
    while (cur->next) {
        ...
        if (((arena_team_t*) team->it)->skin != -1)
            setteamskin(ent, ent->client->pers.userinfo,
                    ((arena_team_t*) team->it)->skin);
    ...
    if (addtoline) {
        if (observe) {
            add_to_queue(&(((arena_team_t*) (team->it))->ar),
                    &arenas[arena].unassigned);
        } else {
            add_to_queue(&(((arena_team_t*) (team->it))->ar),
...
// clear their total score when they move back to the lobby
    ...
    gi.dprintf("%d: %d %s entered\n", arena,
            ((arena_team_t*) (team->it))->teamnum,
            ((arena_team_t*) (team->it))->teamname);
    ...
    if (!observeonly) {
        ...
        if (arenas[arena].settings.minping) {
            if (ent->client->ping < arenas[arena].settings.minping) {
                menu_centerprint(ent,
                        va(
                                "Your ping is too low\nMinimum ping for this arena: %d",
                                arenas[arena].settings.minping));
                return (1);
            }
        }
        if (arenas[arena].settings.maxping) {
            if (ent->client->ping > arenas[arena].settings.maxping) {
                menu_centerprint(ent,
                        va(
                                "Your ping is too high\nMaximum ping for this arena: %d",
                                arenas[arena].settings.maxping));
                return (1);
            }
        }
        //check lock
        ...
        if (arenas[arena].pickup) {
            menu_centerprint(ent,
                    "You must join a pickup team to\n enter that arena");
            return (1);
        }

        if (count_queue(&arenas[arena].unassigned)
                + count_queue(&arenas[arena].players)
                >= arenas[arena].max_teams) {
            menu_centerprint(ent, "Sorry, that arena is full");
            return (1);
        }
    }
    tcount = count_queue(&teams[ent->client->resp.team]);
    if ((tcount == arenas[arena].settings.needed_players_team)
            || (tcount <= arenas[arena].settings.needed_players_team && force)) {
        ((arena_team_t*) (teams[ent->client->resp.team].it))->observeonly =
                observeonly;
        if (!observeonly) {
            remove_from_queue(
                    &(((arena_team_t*) (teams[ent->client->resp.team].it))->ar),
                    NULL); //take them out of arenas[0].unassigned line
        ...
        return 0;
    } else if (count_queue(&teams[ent->client->resp.team])
            < arenas[arena].settings.needed_players_team) {
...
//now, see if all the teams in the arena have the correct # of players
    ...
    while (t->next) //cycle teams
    {
        ...
        if (!arenas[context].pickup) //dont check pings on pickup
            while (p->next) //cycle players, check pings
            {
                ...
                if ((aping > arenas[context].settings.maxping && aping < 1000)
                        || //dont kick out because of bad lag
                        aping < arenas[context].settings.minping) {
                    badping = true;
                    gi.cprintf(((edict_t*) p->it), PRINT_HIGH,
                            "Sorry, your ping of %d does not work in this arena\n",
                            aping);
                }
            }

        if (/*count_queue(t->it) == 0 || */(count_queue(t->it)
                > arenas[context].settings.needed_players_team) || badping) //t->it is the actual team
                {
            gi.bprintf(PRINT_MEDIUM, "Removing team %d (%s)\n",
                    ((arena_team_t*) ((arena_link_t*) t->it)->it)->teamnum,
                    ((arena_team_t*) ((arena_link_t*) t->it)->it)->teamname);
            hold = t->prev;
    ...
    if (arenas[context].settings.changed)
        if (count_queue(&arenas[context].unassigned)
                + count_queue(&arenas[context].players) == 0) //arena is empty
                {
    ...
    for (i = 0; i < maxclients->value; i++) {
        ...
        if (e->inuse && e->client && e->client->resp.context == context) {
            if (arenas[context].game_state == GAME_PRECOUNT)
                send_configstring(e, CS_ARENASTRINGS,
            ...
            if (arenas[context].settings.rounds > 1)
                send_configstring(e, CS_ARENASTRINGS + 1,
                        va("Round %d of %d", arenas[context].round,
                                arenas[context].settings.rounds));
            else
                send_configstring(e, CS_ARENASTRINGS + 1, "");
            ...
            if (count > 0 && count < 4
                    && arenas[context].game_state != GAME_PRECOUNT) {
...
qboolean fill_arena(int context) {
    ...
    for (i = 0; i < arenas[context].needed_teams; i++) {
        ...
        if (t == NULL) { //team left during a multi round match
        ...
        if (lastskin == -1)
            lastskin = ((arena_team_t*) (((arena_link_t*) (t->it))->it))->skin;
                ...
                == ((arena_team_t*) (((arena_link_t*) (t->it))->it))->skin) {
            gi.dprintf("Skin conflict in arena %d\n", context);
            ((arena_team_t*) (((arena_link_t*) (t->it))->it))->skin = (lastskin
                    + 1) % NUMSKINS;
        }
    ...
    while (t->next) //cycle teams
    {
        ...
        while (p->next) //cycle players
        {
            ...
            if (ent->takedamage == DAMAGE_AIM && ent->deadflag == DEAD_NO) {
                ...
                else if (aliveteam != ent->client->resp.team)
...
void CTFSetIDView(edict_t *ent) {
    ...
    tr = gi.trace(ent->s.origin, NULL, NULL, forward, ent,
            CONTENTS_MONSTER | CONTENTS_SOLID); //<-- people can't see through walls (including observer walls)
...
//  frags
    strcpy(my_statusbar,
            "xl 8 yb -10 string2 \"Line Position:\" xl 100 yb -24 num 2 19 ");

    pos = my_statusbar + strlen(my_statusbar);
    for (tc = 0; tc <= tcount; tc++) {
        sprintf(pos, "xl %d yt %d string2 \"%s\" ", xp, yp, teamnames[tc]);
        pos = my_statusbar + strlen(my_statusbar);
        yp += 8;

        for (pc = 0; pc <= pcount[tc]; pc++) {
            sprintf(pos, "xl %d yt %d string2 \"%s: %d\" ", xp, yp,
                    playernames[tc][pc], playerhealth[tc][pc]);
            pos = my_statusbar + strlen(my_statusbar);
            yp += 8;
        }
        yp += 8;
    ...
    for (i = 0; i < maxclients->value; i++) {
        ...
        if (e->inuse && e->client && e->client->resp.context == context
            ...
            if (e->client->resp.track_targ || e->client->showscores) //if tracking someone, or scoreboard or menu is up
                ...
                e->client->ps.stats[STAT_ARENA_POS] =
                        show_rank(
                                &((arena_team_t*) (teams[e->client->resp.team].it))->ar);
    ...
    for (i = 0; i < maxclients->value; i++) {
        ...
        if (e->inuse && e->client && e->client->resp.context == context
            ...
            if (e->client->resp.telefrag_time
                ...
                tr = gi.trace(e->s.origin, e->mins, e->maxs, e->s.origin, NULL,
                        MASK_PLAYERSOLID);
                if (tr.contents == CONTENTS_SOLID) {// rematerialized in a platform?
                    e->solid = SOLID_NOT;
                    tv1[YAW] = rand() % 360;
                    tv1[PITCH] = 0;
                    tv1[ROLL] = 0;
...
void start_voting(edict_t *e, int arena) {
    ...
    if (arenas[arena].game_state == GAME_ON
            || arenas[arena].game_state == GAME_COUNT)
    ...
    arenas[arena].voteinfo.numvoters = arenas[arena].voteinfo.ncount =
            arenas[arena].voteinfo.ycount = 0;
...
void check_voting(int arena) {
    ...
    if ((arenas[arena].voteinfo.ycount - arenas[arena].voteinfo.ncount)
            >= (float) arenas[arena].voteinfo.numvoters / 3.0) { //yes votes - no votes > 1/3 total
        memcpy(&arenas[arena].settings, &arenas[arena].voteinfo.propsettings,
...
void arena_think(int context) {
    ...
    if (a->game_state == GAME_COUNT || a->game_state == GAME_PRECOUNT) { // print countdown to everyone
        if (!a->game_count_frame) { // need to assign the countdown
            a->game_count_frame = level.framenum + 1 / FRAMETIME; //count in 1 sec
            if (a->game_state == GAME_PRECOUNT)
                a->game_count = 15;
            else if (a->voteinfo.endtime > level.time)
        ...
        UpdateStatusBars(context);
        if (!a->game_count_frame) { //wait 3 sec for stuff to blow up
        ...
        if (winner == -1) {
            ...
            if (++((arena_team_t*) (teams[winner].it))->wins
                    > arenas[context].settings.rounds / 2) {
                Com_sprintf(a->winstring, sizeof(a->winstring),
                        "%s has won the match!!",
                        ((arena_team_t*) (teams[winner].it))->teamname);
        ...
        if (winner == -1 && count_queue(&arenas[context].players) != 0
                && count_queue(
                        ((arena_link_t*) (arenas[context].players.next))->it)) //don't increment round number on tie, unless arena is empty, or first team is empty (pickup)
            a->round--;
        ...
        while (pos->next) //cycle teams..
        {
            ...
            if (a->round < arenas[context].settings.rounds) //multiple rounds
                    {
                Com_sprintf(a->winstring, sizeof(a->winstring),
                        "%s has %d wins and needs %d more to take the match\n",
                        ((arena_team_t*) ((arena_link_t*) pos->it)->it)->teamname,
                        ((arena_team_t*) ((arena_link_t*) pos->it)->it)->wins,
                        arenas[context].settings.rounds / 2 + 1
                                - ((arena_team_t*) ((arena_link_t*) pos->it)->it)->wins);
                ...
                ((arena_team_t*) (((arena_link_t*) (t->it))->it))->in_arena =
                        false;
                pos = &arenas[context].players; //goback to start
                if (((arena_team_t*) ((arena_link_t*) t->it)->it)->teamnum
                        == winner || winner == -1) {
...
void CTFResetGrapple(edict_t *self) {
    if (self->owner->client->ctf_grapple) {
        ...
        gi.sound(self->owner, CHAN_RELIABLE + CHAN_WEAPON,
                gi.soundindex("weapons/grapple/grreset.wav"), volume, ATTN_NORM,
                0);
    ...
    if (self->enemy) {
        ...
        if (self->enemy->takedamage
            ...
            T_Damage(self->enemy, self, self->owner, self->velocity,
                    self->s.origin, vec3_origin, 1, 1, 0, MOD_UNKNOWN);
