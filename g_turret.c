...
void turret_breach_think(edict_t *self) {
    ...
    if (self->owner) {
        ...
        angle *= (M_PI * 2 / 360);
        target[0] = SnapToEights(
                self->s.origin[0] + cos(angle) * self->owner->move_origin[0]);
        target[1] = SnapToEights(
                self->s.origin[1] + sin(angle) * self->owner->move_origin[0]);
        ...
        target_z = SnapToEights(
                self->s.origin[2] + self->owner->move_origin[0] * tan(angle)
