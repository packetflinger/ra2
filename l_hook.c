...
//
// Code from Orange 2 Mod
// Modified by Matt "WhiteFang" Ayres for Lithium 2 Mod
...
void Hook_InitGame(void) {
    ...
    hook_damage = lvar("hook_damage", "2", "##", VAR_HOOK);
}
...
void Hook_Think(edict_t *self) {
    ...
    if (!(self && self->owner && self->owner->owner
...
qboolean Hook_Check(edict_t *self) {
    ...
    if (!((self->owner->client->latched_buttons | self->owner->client->buttons)
            & BUTTON_ATTACK)
            && (strcmp(self->owner->client->pers.weapon->pickup_name, "Hook")
                    == 0)) {
        Hook_Reset(self);
        return true;
    }
...
void Hook_Track(edict_t *self) {
    ...
    if (self->enemy->client) {
        ...
        T_Damage(self->enemy, self, self->owner, vec3_origin,
                self->enemy->s.origin, normal, hook_damage->value, 0,
                DAMAGE_NO_KNOCKBACK, MOD_HOOK);
    } else {
        ...
        if (hook_maxtime->value
                && level.time - self->owner->hook_time > hook_maxtime->value) {
            Hook_Reset(self);
            return;
        }
    }
...
void Hook_Touch(edict_t *self, edict_t *other, cplane_t *plane,
    ...
    if (other->client) {		// we hit a player
        ...
        T_Damage(other, self, self->owner, dir, self->s.origin, normal,
                hook_damage->value * 5, hook_damage->value * 5, 0, MOD_HOOK);
    } else {					// we hit something thats not a player
        // if we can hurt it, then do a bit of damage
        if (other->takedamage) {
            _VectorSubtract(other->s.origin, self->owner->s.origin, dir);
            _VectorSubtract(self->owner->s.origin, other->s.origin, normal);
            T_Damage(other, self, self->owner, dir, self->s.origin, normal, 1,
                    1, 0, 0);
        }
        // stop moving
...
void Weapon_Hook_Fire(edict_t *ent) {
    ...
    if (level.time < ent->client->last_hook_time + 1.2)
        return;
    ...
    gi.sound(ent, CHAN_WEAPON, gi.soundindex("flyer/Flyatck3.wav"), 1,
            ATTN_NORM, 0);
