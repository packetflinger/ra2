...
void SpawnEntities(char *mapname, char *entities, char *spawnpoint) {
    ...
    // arena
    //arena_init(G_Find(NULL,FOFS(classname), "worldspawn"));
    arena_init(g_edicts);
...
// arena
    {
        ...
        for (i = 0; i < NUMSKINS; i++) {
            teamskins_precachem[i] = gi.imageindex(
                    va("male/%s_i", teamskins[i]));
            teamskins_precachef[i] = gi.imageindex(
                    va("female/%s_i", teamskins[i]));
            teamskins_precachecw[i] = gi.imageindex(
                    va("crakhor/%s_i", teamskins[i]));
            teamskins_precachecb[i] = gi.imageindex(
                    va("cyborg/%s_i", teamskins[i]));
        }
    }
