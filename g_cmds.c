	...
	*/
	...
	if (toworld || team)
		for (j = 1; j <= game.maxclients; j++)
		{
			...
			if (team)
			{
				if (!OnSameTeam(ent, other) || other->client->resp.flag != ent->client->resp.flag)
					continue;
			}
...
*/	else if (Q_stricmp (cmd, "listkeys") == 0)
	...
	else if (Q_stricmp (cmd, "play") == 0) ; //if they have sound turned off play commands get sent to server
