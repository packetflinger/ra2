...
void P_DamageFeedback(edict_t *player) {
    ...
    if ((level.time > player->pain_debounce_time)
        ...
        if (player->health < 25)
            ...
            l = 100;
        gi.sound(player, CHAN_VOICE, gi.soundindex(va("*pain%i_%i.wav", l, r)),
                1, ATTN_NORM, 0);
    }
...
// arena
        if (arenas[ent->client->resp.context].settings.fallingdamage)
...
void P_WorldEffects(void) {
    ...
    if (waterlevel == 3) {
        ...
        if (breather || envirosuit) {
            ...
            if (((int) (current_client->breather_framenum - level.framenum) % 25)
                    == 0) {
...
void G_SetClientFrame(edict_t *ent) {
    ...
    //arena ### Hentai ### BEGIN
    if (client->anim_priority == ANIM_REVERSE && client->vwepmodel) {
...
void ClientEndServerFrame(edict_t *ent) {
    ...
    //
    //arena - dont let it use new view
    ...
    if (!MenuThink(ent)) //dont showscores if we sent the menu
